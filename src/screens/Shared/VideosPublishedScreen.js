...
export function VideosPublishedScreen(props) {
    ...
    useEffect(() => {
        (
            async () => {
                try {
                    const response = await videoApi.getVideoUser(accessToken, params.idUser)
                    setVideos(response)
                } catch (error) {
                    console.error(error)
                }
    ...
    return (
        <FlatList
            ...
            renderItem={({ item, index }) => (<VideoFeed style={{ height: height }} item={item} index={index} indexShow={indexStart} />)}
